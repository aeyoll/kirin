...
- `ui`: page title, optional organisation string, and `default_locale` (`en`, `fr`, or `ja`, default `en`).

### Language

Supported locales are `en`, `fr`, and `ja`. If the browser sends no `kirin_locale` cookie, the UI language follows `Accept-Language` when it starts with English, French, or Japanese; otherwise `ui.default_locale` is used. Visitors can switch language with the selector in the header; the choice is stored in the `kirin_locale` cookie (set only via `POST /locale`).
